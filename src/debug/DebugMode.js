...
export function createDebugMode({ camera, target, domElement, scene, objects }) {
  if (new URLSearchParams(window.location.search).get('debug') !== '1') return null
  const orbit = new OrbitControls(camera, domElement); orbit.enableDamping = true; orbit.enabled = false; orbit.target.copy(target)
  ...
  const hud = document.createElement('pre'); hud.className = 'debug-hud'; hud.setAttribute('aria-hidden', 'true'); document.body.append(hud)
